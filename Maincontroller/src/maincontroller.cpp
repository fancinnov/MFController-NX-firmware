...
void update_dataflash(void){
	...
	if(addr_num_max<=0){
		...
		dataflash->set_param_vector3f(param->mag2_offsets.num, param->mag2_offsets.value);
		...
		* *************************************************/
		...
		dataflash->get_param_vector3f(param->mag2_offsets.num, param->mag2_offsets.value);
...
void opticalflow_update(void){
	...
	flow_alt=constrain_float(flow_alt, 0.0f, 200.0f);
...
static Vector3f lidar_offset=Vector3f(0.0f,0.0f, 0.0f);//cm FRD坐标系
				...
				switch(cmd.command){
					...
					case MAV_CMD_PREFLIGHT_CALIBRATION:
						if(is_equal(cmd.param1,1.0f)){					//start accel calibrate
							...
							param->mag2_offsets.value={0.0f,0.0f,0.0f};
...
void send_mavlink_heartbeat_data(void){
	...
	battery_status.voltages[1]=(uint16_t)(get_power_volt()*1000);
	battery_status.current_battery=(int16_t)(get_power_current()*100);
...
void update_accel_gyro_data(void){
	imu_healthy=true;
	accel.x = adis16470_data.accf.x;//m/s/s
	accel.y = adis16470_data.accf.y;//m/s/s
	accel.z = adis16470_data.accf.z;//m/s/s

	if((is_equal(accel.length(),0.0f)) || isnan(accel.x) || isinf(accel.x) || isnan(accel.y) || isinf(accel.y) || isnan(accel.z) || isinf(accel.z)){
		imu_healthy=false;
		accel.x = icm42688_data.accf.x;//m/s/s
		accel.y = icm42688_data.accf.y;//m/s/s
		accel.z = icm42688_data.accf.z;//m/s/s
		if((is_equal(accel.length(),0.0f)) || isnan(accel.x) || isinf(accel.x) || isnan(accel.y) || isinf(accel.y) || isnan(accel.z) || isinf(accel.z)){
			accel.x = icm20689_data.accf.x;//m/s/s
			accel.y = icm20689_data.accf.y;//m/s/s
			accel.z = icm20689_data.accf.z;//m/s/s
			if((is_equal(accel.length(),0.0f)) || isnan(accel.x) || isinf(accel.x) || isnan(accel.y) || isinf(accel.y) || isnan(accel.z) || isinf(accel.z)){
				return;
			}
		}
...
//	usb_printf("ax:%f,ay:%f,az:%f\n",accel.x,accel.y,accel.z);
	...
	gyro.x = adis16470_data.gyrof.x;//rad/s
	gyro.y = adis16470_data.gyrof.y;//rad/s
	gyro.z = adis16470_data.gyrof.z;//rad/s
	if( isnan(gyro.x) || isinf(gyro.x) || isnan(gyro.y) || isinf(gyro.y) || isnan(gyro.z) || isinf(gyro.z)){
		imu_healthy=false;
		gyro.x = icm42688_data.gyrof.x;//rad/s
		gyro.y = icm42688_data.gyrof.y;//rad/s
		gyro.z = icm42688_data.gyrof.z;//rad/s
		if( isnan(gyro.x) || isinf(gyro.x) || isnan(gyro.y) || isinf(gyro.y) || isnan(gyro.z) || isinf(gyro.z)){
			gyro.x = icm20689_data.gyrof.x;//rad/s
			gyro.y = icm20689_data.gyrof.y;//rad/s
			gyro.z = icm20689_data.gyrof.z;//rad/s
			if( isnan(gyro.x) || isinf(gyro.x) || isnan(gyro.y) || isinf(gyro.y) || isnan(gyro.z) || isinf(gyro.z)){
				return;
			}
		}
...
#if USE_MAG
	mag.x = rm3100_data.magf.x;
	mag.y = rm3100_data.magf.y;
	mag.z = rm3100_data.magf.z;
//	usb_printf("mag:%f|%f|%f|%f|%f|%f\n",rm3100_data.magf.x,qmc5883_data.magf.x,rm3100_data.magf.y,qmc5883_data.magf.y,rm3100_data.magf.z,qmc5883_data.magf.z);

	if(isnan(mag.x) || isinf(mag.x) || isnan(mag.y) || isinf(mag.y) || isnan(mag.z) || isinf(mag.z) || (!use_mag2)){
		...
		mag_correct=mag+param->mag2_offsets.value;
		if(is_equal(param->mag2_offsets.value.x,0.0f)||is_equal(param->mag2_offsets.value.y,0.0f)||is_equal(param->mag2_offsets.value.z,0.0f)){
			return;
		}
	}
...
void compass_calibrate(void){
	...
	if(is_equal(completion_percent,1.0f)){
		if(use_mag2){
			compassCalibrator->get_calibration(param->mag2_offsets.value);
			dataflash->set_param_vector3f(param->mag2_offsets.num, param->mag2_offsets.value);
		}else{
...
void sdled_update(void){
	if(initial_accel_gyro&&initial_mag&&initial_baro&&imu_healthy){
		Led_Red_Control(false);
		if(get_soft_armed()){
			Led_Green_Control(true);
			osDelay(500);
			if(sdlog->m_Logger_Status!=SDLog::Logger_Record){
				Led_Green_Control(false);
			}
		}else{
			if(motors->get_interlock()){
				Led_Green_Control(true);
...
void Logger_Cat_Callback(void){
	...
	sd_log_write("%8s %8s %8s %8s %8s %8s %8s ",//LOG_EULER
			"pitchr", "rollr", "yawr", "pitchd", "rolld", "yawd", "rtk_yawd");
	osDelay(1);
	...
	osDelay(1);
	sd_log_write("%8s %8s %8s %8s %8s %8s %8s %8s %8s ",//LOG_POS_XY
			"vt_z","odom_x", "ned_x", "pos_x", "vel_x", "odom_y", "ned_y", "pos_y", "vel_y");
	osDelay(1);
...
void Logger_Data_Callback(void){
	...
	sd_log_write("%8.3f %8.3f %8.3f %8.3f %8.3f %8.3f %8d ",//LOG_SENSOR
			get_mag_filt().x, get_mag_filt().y, get_mag_filt().z, spl06_data.baro_alt, get_power_volt(), get_power_current(), gps_position->satellites_used);
	osDelay(1);
	sd_log_write("%8.3f %8.3f %8.3f %8.3f %8.3f %8.3f %8.3f ",//LOG_EULER
			ahrs_pitch_rad(), ahrs_roll_rad(), ahrs_yaw_rad(), ahrs_pitch_deg(), ahrs_roll_deg(), ahrs_yaw_deg(), gps_position->heading);
	osDelay(1);
