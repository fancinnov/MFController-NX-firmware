...
void HeartbeatTask(void *argument)
{
  ...
  for(;;)
  {
	  Led_Blue_Control(true);
	  send_mavlink_heartbeat_data();
	  osDelay(100);
	  Led_Blue_Control(get_gnss_state());
	  sdled_update();
...
/* USER CODE BEGIN Header_BaroTask */
/**
* @brief Function implementing the baroTask thread.
* @param argument: Not used
* @retval None
*/
/* USER CODE END Header_BaroTask */
void BaroTask(void *argument)
{
  /* USER CODE BEGIN BaroTask */
	while(!initialed_task){
		osDelay(1000);
	}
	const TickType_t TimeIncrement= pdMS_TO_TICKS(10); // 10ms
  ...
  {
	vTaskDelayUntil(&PreviousWakeTime, TimeIncrement);
	MS5611_Enable_Press_Conv();
	osDelay(3);
	MS5611_GetD1();
	MS5611_Enable_Temp_Conv();
	osDelay(3);
	MS5611_GetD2();
	MS5611_Get_Data();
    BARO_Get_Date();
  }
  /* USER CODE END BaroTask */
}
